...
test("the memory can be corrected by hand", async ({ tab }) => {
  ...
  await tab.getByTestId("memory-editor").fill("- The supervisor is Prof. Vellacourt");
  await tab.getByTestId("memory-save").click();
  await expect(tab.getByTestId("memory-text")).toContainText("Vellacourt");
});
...
test("the agent sheet can be looked at and left, from the bar and by Escape", async ({ app, page }) => {
  // Reported by the user, of the screen this sheet replaced: there was no
  // way off it except choosing something.  Over an agent that works, the
  // sheet has Cancel, "Keep Claude" and Escape, and all three leave the
  // choice alone.
  //
  ...
  await expect(setup).toHaveText(/Claude/);
  await setup.click();
  const sheet = page.getByRole("dialog", { name: "What writes with you" });
  ...
  // Into another row, and Escape leaves the sheet with the choice unmade.
  await setup.click();

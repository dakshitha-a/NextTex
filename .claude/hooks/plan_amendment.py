...
Claude Code runs this from three hooks, and the model runs it once by hand.

`started`, from `PostToolUse` on `ExitPlanMode`, writes a marker saying
this session is executing the plan the user just approved.  `prompt`, from
...
plan mode, merge it, validate, get it approved, resume.  `session`, from
`SessionStart`, names any plan an earlier session left unfinished.  `done`
is what the model runs when the plan is complete, and removes the marker.

The reason for a hook rather than a rule is that the rule already existed
and the tasks were still being dropped.  A rule lives in the model's
context, which is at its longest and most crowded exactly when a plan has
been running for a while; a hook fires from the harness on every message,
...
HOW_TO_FINISH = (
    "When every item in the plan is done and verified, run "
    "`python3 .claude/hooks/plan_amendment.py done {session}` so this "
    "reminder stops."
)
...
def started(data: dict) -> None:
    ...
    if record["amendments"]:
        text = (
            f"The amended plan at {relative(plan)} is approved.  Resume from "
            "the item you were on when the amendment arrived."
        )
    else:
        text = (
            f"The plan at {relative(plan)} is approved and this session is now "
            "executing it.  The plan file is the ledger: keep a state on every "
            "item there as you go.  Any message the user sends before it is "
...
def session(data: dict) -> None:
    ...
    respond("SessionStart", (
        ...
        + "; ".join(left)
        + ".  Before starting new work, read it, compare its items against "
        "the git log, and tell the user what is unfinished.  If it is in "
        "fact complete, remove its marker with "
        "`python3 .claude/hooks/plan_amendment.py done <session id>`."
    ))
